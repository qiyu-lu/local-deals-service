...
```text
benchmark/seckill/seckill-rocketmq-v2/2026-08-15-seckill-rocketmq-reservation-v2-100t-1l-summary-r1.csv
```

Aggregate Report 导出文件建议命名：

```text
benchmark/seckill/seckill-rocketmq-v2/2026-08-15-seckill-rocketmq-reservation-v2-100t-1l-aggregate-r1.csv
```
...
- 通过命令行 JMeter 运行 `docs/testing/seckill-benchmark.jmx`。
- 生成 JTL 原始文件、HTML Dashboard、Summary CSV 和 Aggregate CSV。
- JMeter 结束后轮询 MySQL 订单与 Redis `SUCCESS` 终态，记录异步落库和状态收敛耗时 `drain_ms`。
- 输出机器可读总表：`benchmark/seckill/<场景名>/metrics.csv`。
- 输出单轮证据快照：`benchmark/seckill/<场景名>/<run_id>-run-summary.md`。
...
benchmark/seckill/seckill-rocketmq-v2/2026-08-15-seckill-rocketmq-reservation-v2-5000t-5l-summary-r1.csv
benchmark/seckill/seckill-rocketmq-v2/2026-08-15-seckill-rocketmq-reservation-v2-5000t-5l-aggregate-r1.csv
benchmark/seckill/seckill-rocketmq-v2/2026-08-15-seckill-rocketmq-reservation-v2-5000t-5l-r1-run-summary.md
benchmark/seckill/seckill-rocketmq-v2/metrics.csv
```
...
`summary.csv` 和 `aggregate.csv` 是 JMeter 派生结果；日常阅读优先看同一运行目录中的 `metrics.csv` 和 `run-summary.md`。这些生成物默认被 Git 忽略，只有完成环境、参数和业务不变量审查后，才应在独立阶段形成正式 evidence 文档。
...
```bash
mkdir -p benchmark
jmeter -n -t "docs/testing/seckill-benchmark.jmx" -l benchmark/seckill-smoke.jtl
```
...
jmeter -n \
  -t "docs/testing/seckill-benchmark.jmx" \
...
```text
benchmark/seckill/<场景名>/
```

正式证据文档应引用经复核的运行结果，并记录：
