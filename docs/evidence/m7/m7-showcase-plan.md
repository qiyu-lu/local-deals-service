...
- M5D `m5d-20260822a` 是 Redis 短时不可用、MySQL 不可用、consumer pause、ES 不可用和
  Broker 不可用的正式故障证据；直接引用 `docs/evidence/m5/m5d-reliability-results.md` 和机器摘要，保留
...
- `docs/evidence/m7/m7-evidence-index.md`：统一入口、证据级别、系统边界图和秒杀状态图。
- `docs/evidence/m7/m7-failure-matrix.csv`：固定列数的机器可读故障矩阵。
- `docs/guides/project-demo.md`：15 分钟演示顺序、命令、查询证据和清理边界。
- `docs/evidence/m7/m7-results.md`：最终结果、已解决/仍有边界/何时升级和本地恢复时间声明。
