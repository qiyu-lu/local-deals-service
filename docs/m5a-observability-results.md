...
本结果只描述当前实现。它不证明系统容量提升，也不代表有界缓存、429 限流或自动降级已经完成。小型机器可读结果见 `docs/m5a-observability-summary.csv`；JTL、Prometheus 快照、日志和容器数据保留在已忽略的 `benchmark/m5a/20260820m5a/`，不进入 Git。
...
指标目录见 `docs/m5a-metric-catalog.md`。实际 Prometheus 文本确认存在：
