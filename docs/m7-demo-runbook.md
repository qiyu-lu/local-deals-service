...
打开 [`m7-evidence-index.md`](m7-evidence-index.md)，先讲系统图：
...
证据来源：M5/M6 的安全测试和 [`admin-rbac.md`](admin-rbac.md)。这是“集成测试证据 +
...
打开 [`m7-failure-matrix.csv`](m7-failure-matrix.csv) 和 [`m7-results.md`](m7-results.md)，
快速说明：
...
sed -n '1,220p' docs/m7-evidence-index.md
column -s, -t < docs/m7-failure-matrix.csv | sed -n '1,18p'
```
