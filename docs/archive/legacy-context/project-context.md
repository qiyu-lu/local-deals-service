...
| `ca1c5df` | docs/archive/legacy-context/improvement-comparison.md |
...
2. 面试讲法准备（参考 `docs/archive/legacy-context/superpowers/specs/2026-06-30-es-rocketmq-websocket-design.md` 的”面试叙述要点”）
...
- 新增 `docs/archive/legacy-context/improvement-comparison.md` — before/after 对比数据
...
- 补充 `docs/archive/redis-stream/benchmark-results.md` 和 `docs/archive/redis-stream/reliability-results.md` 的表格阅读说明：解释 `Throughput`、`P95/P99`、`drain_ms`、`pending`、`dead-letter`、`retry_key_count` 等字段，并明确 baseline 正常压测通过不等于异常链路可靠。
...
- 新增 `docs/archive/redis-stream/reliability-results.md` 作为故障注入人工总览；更新 `docs/archive/redis-stream/benchmark-results.md` 和 `docs/archive/redis-stream/seckill-comparison-test-runbook.md`，将本轮对比状态从“待执行”改为“已补齐”。
- 更新 README 首页表达，把“相比教程原型的核心优势”和“对比验证摘要”前置，明确优势是可靠性闭环、可观测性和自动化验证，而不是性能大幅提升。
- 新增 `docs/archive/redis-stream/seckill-comparison-test-runbook.md` 作为秒杀对比验证恢复入口，记录 baseline worktree、数据库差异、正常压测命令、故障注入命令、证据字段、通过标准和当前中断点。
...
- 本次收尾决策：今天暂不继续新增代码，停止在已推送的 `a9ae77a Repackage local deals seckill reliability` 之后；原计划中的故障注入脚本和 `docs/archive/redis-stream/reliability-results.md` 尚未实现。
...
- `reliable-stream-v1` 结果已写入 `docs/archive/redis-stream/JmeterTestSummary/seckill-reliable-v1/` 和 `docs/archive/redis-stream/benchmark-results.md`：100、1000、5000、25000 请求四组均 `pass`，pending 为 0，dead-letter 为 0，`drain_ms` 为 72-80 ms。
...
- 参数化 `docs/testing/seckill-benchmark.jmx`，支持通过脚本参数控制线程数、循环次数、库存、用户数、优惠券 ID 等压测变量。
- 清理旧的手工压测文件，只保留当前脚本生成的结果入口和证据文件。
- 更新 `docs/archive/redis-stream/benchmark-results.md`，将它定位为人工阅读总览表，不再堆积所有原始输出。
...
结果以 `docs/archive/redis-stream/benchmark-results.md` 和 `docs/archive/redis-stream/JmeterTestSummary/seckill-baseline-lua-stream/metrics.csv` 为准。
...
- `docs/archive/redis-stream/benchmark-results.md`
  - 人工阅读入口，只保留压测总览表和简短观察。
- `docs/archive/redis-stream/JmeterTestSummary/seckill-baseline-lua-stream/metrics.csv`
  - 机器可读总表，每轮脚本压测自动更新。
- `docs/archive/redis-stream/JmeterTestSummary/seckill-reliable-v1/metrics.csv`
  - 可靠性增强版机器可读总表。
- `docs/archive/redis-stream/reliability-results.md`
  - 故障注入人工总览，记录 baseline pending 残留和 current 进入 DLQ 的对比结论。
- `docs/archive/redis-stream/reliability-results/metrics.csv`
  - 故障注入机器可读总表。
- `docs/archive/redis-stream/JmeterTestSummary/seckill-baseline-lua-stream/*-run-summary.md`
  - 每轮压测的完整证据快照。
- `docs/archive/redis-stream/JmeterTestSummary/seckill-reliable-v1/*-run-summary.md`
  - 可靠性增强版每轮压测的完整证据快照。
...
- `docs/testing/seckill-benchmark.jmx`
  - JMeter 脚本已参数化，支持通过 `-Jthreads`、`-Jloops`、`-JvoucherId`、`-JtokensFile` 等参数运行。
- `docs/guides/seckill-benchmark.md`
  - 记录自动化脚本用法、参数含义、输出文件命名和 `drain_ms` 含义。
- `docs/archive/redis-stream/seckill-comparison-test-runbook.md`
...
```text
请先阅读 /home/sd101t/IdeaProjects/hm-dianping/docs/archive/legacy-context/project-context.md，
再阅读 /home/sd101t/IdeaProjects/hm-dianping/docs/archive/redis-stream/seckill-comparison-test-runbook.md，
...
已完成 baseline/current 的 1000/5000 正常压测和故障注入对比；结果入口是 docs/archive/redis-stream/benchmark-results.md 和 docs/archive/redis-stream/reliability-results.md。
