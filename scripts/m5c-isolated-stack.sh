...
ACTION="${1:-}"
LABEL_KEY="com.localdeals.m5c.run-id"

fail() {
  echo "M5C isolation gate failed: $*" >&2
  exit 1
}
...
require_isolation() {
  ...
  require_env M5C_RUN_ID
  [[ "$M5C_RUN_ID" =~ ^[a-z0-9][a-z0-9-]{5,30}$ ]] ||
    fail "M5C_RUN_ID must match [a-z0-9][a-z0-9-]{5,30}"
  require_env M5C_APP_HOST
  [[ "$M5C_APP_HOST" == "127.0.0.1" ]] || fail "M5C_APP_HOST must be 127.0.0.1"

  local name
  for name in M5C_APP1_PORT M5C_MANAGEMENT1_PORT M5C_APP2_PORT M5C_MANAGEMENT2_PORT \
      M5C_MYSQL_PORT M5C_REDIS_PORT M5C_ES_PORT M5C_RMQ_NAMESRV_PORT \
      M5C_RMQ_BROKER_PORT; do
    require_env "$name"
    require_port "$name"
  done
  require_env M5C_MYSQL_PASSWORD
  require_env M5C_REDIS_PASSWORD
  require_env M5C_RMQ_TOPIC
  require_env M5C_RMQ_CONSUMER_GROUP
  require_env M5C_RMQ_PRODUCER_GROUP

  local scoped
  for scoped in "$M5C_RMQ_TOPIC" "$M5C_RMQ_CONSUMER_GROUP" "$M5C_RMQ_PRODUCER_GROUP"; do
    [[ "$scoped" == *"$M5C_RUN_ID"* ]] || fail "RocketMQ topic/groups must contain M5C_RUN_ID"
  done

  local ports=("$M5C_APP1_PORT" "$M5C_MANAGEMENT1_PORT" "$M5C_APP2_PORT"
    "$M5C_MANAGEMENT2_PORT" "$M5C_MYSQL_PORT" "$M5C_REDIS_PORT" "$M5C_ES_PORT"
    "$M5C_RMQ_NAMESRV_PORT" "$M5C_RMQ_BROKER_PORT" "$((M5C_RMQ_BROKER_PORT + 1))")
  local unique
  unique="$(printf '%s\n' "${ports[@]}" | sort -u | wc -l)"
  [[ "$unique" -eq "${#ports[@]}" ]] ||
...
assert_owned_container() {
  ...
  [[ "$label" == "$M5C_RUN_ID" ]] || fail "container is not owned by this run: $name"
}
...
mysql_ready() {
  docker exec "$(container_name mysql)" mysqladmin ping \
    -uroot "-p${M5C_MYSQL_PASSWORD}" --silent
}

redis_ready() {
  docker exec "$(container_name redis)" redis-cli \
    -a "$M5C_REDIS_PASSWORD" --no-auth-warning PING | grep -qx PONG
}

es_ready() {
  curl --fail --silent "http://127.0.0.1:${M5C_ES_PORT}/_cluster/health" >/dev/null
}
...
write_redis_sentinel() {
  docker exec "$(container_name redis)" redis-cli \
    ...
    SET "m5c:sentinel:${M5C_RUN_ID}" "$M5C_RUN_ID" >/dev/null
}

verify_sentinels() {
  local mysql_value redis_value es_value
  mysql_value="$(docker exec "$(container_name mysql)" mysql -N -s \
    -uroot "-p${M5C_MYSQL_PASSWORD}" "$(schema_name)" -e "SELECT DATABASE()")"
  [[ "$mysql_value" == "$(schema_name)" ]] || fail "MySQL schema identity mismatch"
  redis_value="$(docker exec "$(container_name redis)" redis-cli \
    -a "$M5C_REDIS_PASSWORD" --no-auth-warning --raw \
    GET "m5c:sentinel:${M5C_RUN_ID}")"
  [[ "$redis_value" == "$M5C_RUN_ID" ]] || fail "Redis sentinel mismatch"
  es_value="$(curl --fail --silent "http://127.0.0.1:${M5C_ES_PORT}/" | \
...
provision_rocketmq_scope() {
  ...
  docker exec "$broker" sh mqadmin updateTopic -n "$namesrv" \
    -c "$(cluster_name)" -t "$M5C_RMQ_TOPIC" >/dev/null
  docker exec "$broker" sh mqadmin updateSubGroup -n "$namesrv" \
    -c "$(cluster_name)" -g "$M5C_RMQ_CONSUMER_GROUP" >/dev/null
  docker exec "$broker" sh mqadmin updateSubGroup -n "$namesrv" \
    -c "$(cluster_name)" -g "$M5C_RMQ_PRODUCER_GROUP" >/dev/null
}
...
up() {
  ...
  docker network create --label "${LABEL_KEY}=${M5C_RUN_ID}" "$network" >/dev/null

  docker run -d --name "$mysql_name" --network "$network" \
    --label "${LABEL_KEY}=${M5C_RUN_ID}" \
    -p "127.0.0.1:${M5C_MYSQL_PORT}:3306" \
    -e MYSQL_ROOT_PASSWORD="$M5C_MYSQL_PASSWORD" -e MYSQL_DATABASE="$schema" \
    mysql:8.0 --default-authentication-plugin=mysql_native_password >/dev/null

  docker run -d --name "$redis_name" --network "$network" \
    --label "${LABEL_KEY}=${M5C_RUN_ID}" \
    -p "127.0.0.1:${M5C_REDIS_PORT}:6379" \
    redis:6.2 redis-server --save '' --appendonly no \
    --requirepass "$M5C_REDIS_PASSWORD" >/dev/null

  docker run -d --name "$es_name" --network "$network" \
    --label "${LABEL_KEY}=${M5C_RUN_ID}" \
    -p "127.0.0.1:${M5C_ES_PORT}:9200" \
    -e discovery.type=single-node -e "cluster.name=$(cluster_name)" \
    -e "ES_JAVA_OPTS=-Xms384m -Xmx384m" \
    "${M5C_ES_IMAGE:-hm-dianping-elasticsearch:latest}" >/dev/null

  docker run -d --name "$namesrv_name" --network "$network" \
    --label "${LABEL_KEY}=${M5C_RUN_ID}" \
    -p "127.0.0.1:${M5C_RMQ_NAMESRV_PORT}:9876" \
  ...
  local broker_conf="${artifact_dir}/broker.conf"
  {
    ...
    printf 'brokerName=%s\n' "$(cluster_name)"
    printf 'brokerId=0\nbrokerIP1=127.0.0.1\n'
    printf 'listenPort=%s\nhaListenPort=%s\n' "$M5C_RMQ_BROKER_PORT" "$((M5C_RMQ_BROKER_PORT + 1))"
  ...
  docker run -d --name "$broker_name" --network "$network" \
    --label "${LABEL_KEY}=${M5C_RUN_ID}" \
    -p "127.0.0.1:${M5C_RMQ_BROKER_PORT}:${M5C_RMQ_BROKER_PORT}" \
    -p "127.0.0.1:$((M5C_RMQ_BROKER_PORT + 1)):$((M5C_RMQ_BROKER_PORT + 1))" \
    -v "${broker_conf}:/home/rocketmq/rocketmq-4.9.4/conf/m5c-broker.conf:ro" \
    apache/rocketmq:4.9.4 sh mqbroker \
    -c /home/rocketmq/rocketmq-4.9.4/conf/m5c-broker.conf >/dev/null
  ...
  echo "M5C isolated stack is ready."
  echo "run_id=${M5C_RUN_ID} schema=${schema} cluster=$(cluster_name)"
  echo "apps=127.0.0.1:${M5C_APP1_PORT},127.0.0.1:${M5C_APP2_PORT} management=127.0.0.1:${M5C_MANAGEMENT1_PORT},127.0.0.1:${M5C_MANAGEMENT2_PORT}"
  echo "mysql=127.0.0.1:${M5C_MYSQL_PORT} redis=127.0.0.1:${M5C_REDIS_PORT} elasticsearch=127.0.0.1:${M5C_ES_PORT}"
  echo "ROCKETMQ_NAME_SERVER=127.0.0.1:${M5C_RMQ_NAMESRV_PORT} topic=${M5C_RMQ_TOPIC} consumer_group=${M5C_RMQ_CONSUMER_GROUP} producer_group=${M5C_RMQ_PRODUCER_GROUP}"
}
...
status() {
  ...
  verify_sentinels
  verify_rocketmq_scope
  echo "M5C isolation checks passed."
}
...
consumer_action() {
  local enabled="$1" broker namesrv config
  broker="$(container_name broker)"
  namesrv="$(container_name namesrv):9876"
  assert_owned_container "$broker"
  docker exec "$broker" sh mqadmin updateSubGroup -n "$namesrv" \
    -c "$(cluster_name)" -g "$M5C_RMQ_CONSUMER_GROUP" -s "$enabled" >/dev/null
...
down() {
  ...
  if docker network inspect "$network" >/dev/null 2>&1; then
    ...
    [[ "$label" == "$M5C_RUN_ID" ]] || fail "network ownership mismatch: $network"
    docker network rm "$network" >/dev/null || residual=1
  fi
  if docker ps -a --filter "label=${LABEL_KEY}=${M5C_RUN_ID}" --format '{{.Names}}' | grep -q .; then
    residual=1
  fi
  docker network ls --filter "label=${LABEL_KEY}=${M5C_RUN_ID}" --format '{{.Name}}' | grep -q . && residual=1
  echo "Removed only resources labelled for M5C run ${M5C_RUN_ID}; container data is not recoverable."
  [[ "$residual" -eq 0 ]] || fail "M5C run has residual resources; manual inspection required"
